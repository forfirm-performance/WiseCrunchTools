@echo off
REM ###################################################
REM ##      SCRIPT RIDUZIONE NPLE DI BASE
REM ## RUN SCRIPT TO REDUCE NUMBER BASE NPLE
REM ###################################################
...
IF EXIST "%APP_PATH%%PREFIX_FILE_NAME_OUTPUT_PRODCART%_%WISE%.txt" (
	reduceNple.exe  -n: "%APP_PATH%%PREFIX_FILE_NAME_OUTPUT_PRODCART%_%WISE%.txt"  -s: %SEP%
) else (
    echo[ 
	echo[
