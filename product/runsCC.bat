...
:HELP
	...
	echo[
	echo Usage
	echo    runsCC {FILE TESTSET} {WISE NUMBER} {OPTIONS}
	echo[
...
IF EXIST "%APP_PATH%%PREFIX_FILE_NAME_OUTPUT_PRODCART%_%WISE%.txt" (
	calcolaCoperturaSlow.exe  -w: "%APP_PATH%%PREFIX_FILE_NAME_OUTPUT_PRODCART%_%WISE%.txt"  -t: "%FILE_TESTSET%" -o: %WISE% -s: %SEP% %OPT_01% %OPT_02% %OPT_03% %OPT_04% %OPT_05%
) else (
    echo[ 
	echo[
