...
:HELP
	...
	echo[
	echo Usage
	echo    runTS {FILE TESTSET BASE} {WISE NUMBER} 
	echo[
...
IF NOT EXIST "%FILE_TESTSET_BASE%" (
	...
	start /min /b generaTestSet.exe -w: "%APP_PATH%%PREFIX_FILE_NAME_OUTPUT_PRODCART%_%WISE%.txt" -t: "%FILE_TESTSET_BASE%" -o: %WISE% -s: %SEP%  1>test_set_generato_%WISE%.txt 2>clean_test_set_generato_%WISE%.txt
	echo[ 
	echo[
