...
:HELP
	...
	echo[
	echo Usage
	echo    runsTS {FILE TESTSET BASE} {WISE NUMBER} 
	echo[
...
IF NOT EXIST "%FILE_TESTSET_BASE%" (
    echo[ 
	echo[
	echo Base file - with the test to pick from - "%FILE_TESTSET_BASE%" does not exixts!
	echo[ 
	echo[ 
	GOTO:EOF
)
	...
	taskkill /F /FI "IMAGENAME eq generaTestSetSlow.exe" 1>NUL 2>NUL
	erase test_set_generato_%WISE%.txt clean_test_set_generato_%WISE%.txt 1>NUL 2>NUL
	start /min /b generaTestSetSlow.exe -w: "%APP_PATH%%PREFIX_FILE_NAME_OUTPUT_PRODCART%_%WISE%.txt" -t: "%FILE_TESTSET_BASE%" -o: %WISE% -s: %SEP%  1>test_set_generato_%WISE%.txt 2>clean_test_set_generato_%WISE%.txt
	echo[ 
	echo[
	echo Will be created file 'test_set_generato_%WISE%.txt' and file 'clean_test_set_generato_%WISE%.txt' in the current directory when process 'generaTestSetSlow.exe', visible from TaskManager, will finish.
	echo[ 
	echo[
	goto LOOP

:LOOP	
for /F "delims=*" %%p in ('tasklist /FI "IMAGENAME eq generaTestSetSlow.exe" ^| findstr "generaTestSetSlow.exe" ') do (
	echo[
	echo[
	tasklist /FI "IMAGENAME eq generaTestSetSlow.exe
	echo[
	echo[
	echo Process is running...now is: %time%
	echo[
	echo[
	echo *PLEASE NOTE*: Using CTRL+C, the batch terminate but the process 'generaTestSetSlow.exe' will not be killed and keep running
	echo *NOTA BENE* : Uscendo con CTRL+C, il batch termina ma il processo 'generaTestSetSlow.exe', se attivo, continua a essere eseguito. 
	echo[
	echo To kill the process use the command:
	echo Per eliminare il processo usare il comando: 
	echo     taskkill /F /FI "IMAGENAME eq generaTestSetSlow.exe"
	echo[
	echo or close the process 'generaTestSetSlow.exe' from Task Manager
	echo oppure chiudere il processo 'generaTestSetSlow.exe' da Task Manager 
	echo[
	echo[
...
echo[
echo[
dir "clean_test_set_generato_%WISE%.txt"
dir "test_set_generato_%WISE%.txt"
