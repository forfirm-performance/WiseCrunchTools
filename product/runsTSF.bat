...
:HELP
	...
	echo[
	echo Usage
	echo    runsTSF {FILE TESTSET BASE} {FILE TESTSET PARZIALE} {WISE NUMBER}
	ECHO[
...
IF NOT EXIST "%FILE_TESTSET_PARZIALE%" (
	...
	start /min /b generaTestSetSlow.exe -w: "%APP_PATH%%PREFIX_FILE_NAME_OUTPUT_PRODCART%_%WISE%.txt" -t: "%FILE_TESTSET_BASE%" -o: %WISE% -s: %SEP% -f: "%APP_PATH%vietate_%WISE%.txt"   1>delta_test_set_generato_%WISE%.txt 2>delta_clean_test_set_generato_%WISE%.txt
	echo[ 
	echo[
