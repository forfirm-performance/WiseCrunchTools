...
IF NOT EXIST "%FILE_TESTSET_BASE%" (
	...
	start /min /b generaTestSetSlow.exe -w: "%APP_PATH%%PREFIX_FILE_NAME_OUTPUT_PRODCART%_%WISE%.txt" -t: "%FILE_TESTSET_BASE%" -o: %WISE% -s: %SEP%  1>test_set_generato_%WISE_MAX%_%WISE%.txt 2>clean_test_set_generato_%WISE_MAX%_%WISE%.txt
	echo[ 
	echo[
	echo Will be created file 'test_set_generato_%WISE_MAX%_%WISE%.txt' and file 'clean_test_set_generato_%WISE_MAX%_%WISE%.txt' in the current directory when process 'generaTestSetSlow.exe', visible from TaskManager, will finish.
	echo[ 
	echo[
